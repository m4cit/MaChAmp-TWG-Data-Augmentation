...
_Translates the original Unimorph file into the language / format of the CoNLLU file._

**2_improveUnimorph.py**:
_Looks up all verbs from the Unimorph file successively on dictionary.com, and categorizes them into 'transitive' and 'intransitive' (Very slow. Better to use a dictionary API if available.)._

**3_improveRRG.py**:
_Checks and adds the transitivity of all verbs to the RRG CoNLLU file._

**4_filterForTrain.py**:
_Filters out all unused words / lines in the RRG CoNLLU file._

**generate.py**:
_Replaces words in the RRG CoNLLU file with randomly chosen ones from the UniMorph file._

**augment.py**:
_Replaces original words in the training file with random ones generated by the module 'generate.py', followed by the augmentation of the training file with new sentences._
